...
def run_shiratama_custom(gemini_api_key):
    try:
        ...
        if st.button("アップロードした画像のデータ抽出を実行する"):
            ...
            #補足指示
            同じプレイヤー名が複数回登場しても、**スコアが異なれば「別人」です。名前＋スコアのセットで識別し、重複を避けてください。スコアの違いを認識して、正しい人を「メンバー」シートから抽出してください。**
            ※名前の判定には、1文字違い（例：「ritum」と「yokki_」）などの誤認識が起こらないよう、常に **スコアとのペアで判断**してください。
